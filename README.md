...
클러스터가 만들어지면 마스터 노드에 작업 폴더(`/home/hadoop/works`)가 생기고, Jupyter 노트북에서 작업한 내용은 이 폴더에 저장된다.  이 폴더에 저장된 내용은 *자동으로 이 S3 버킷에 동기되어, 클러스터 제거 후 다시 생성하여도 작업 내용이 그대로 남아있게* 된다.
